...
async function seed() {
  ...
  await mongoose.connect(uri, { dbName: "lawshpere" });
  console.log("Connected to MongoDB (lawshpere).");

  // Demo users used as topic authors + commenters
  const demoUsers = await Promise.all([
    getOrCreateUser({ name: "Aarav Mehta", email: "aarav@lawshpere.test" }),
    getOrCreateUser({ name: "Isha Patel", email: "isha@lawshpere.test" }),
    getOrCreateUser({ name: "Kabir Singh", email: "kabir@lawshpere.test" }),
    getOrCreateUser({ name: "Naina Roy", email: "naina@lawshpere.test" }),
    getOrCreateUser({ name: "Rohan Gupta", email: "rohan@lawshpere.test" }),
  ]);
